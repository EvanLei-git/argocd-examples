...
echo "🚀 Starting high-speed memory-to-memory file sync (Staging → Prod)..."
echo "⏳ This may take a moment depending on the size of your wp-content folder..."

set +e
kubectl exec "$STAGING_POD" -n wordpress -- tar cf - -C /var/www/html wp-content | \
kubectl exec -i "$PROD_POD" -n wordpress -- tar xf - -C /var/www/html
...
echo "🎉 File sync complete! Your production environment now exactly matches staging's files."
